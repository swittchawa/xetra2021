...
class TestS3BucketConnectorMethods(unittest.TestCase):
    ...
    def test_read_csv_to_df_ok(self):
        ...
        self.assertEqual(val2_exp, df_result[col1_exp][0])
        # Cleanup after test
        self.s3_bucket.delete_objects(
            Delete={
                'Objects':[
    ...
    def test_write_df_to_s3_csv(self):
        ...
        with self.assertLogs() as logm:
            ...
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
        data = self.s3_bucket_conn.Object(key=key_exp).get().get('Body').read().decode('utf-8')
    ...
    def test_write_df_to_s3_parquet(self):
        ...
        with self.assertLogs() as logm:
            ...
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
        data = self.s3_bucket_conn.Object(key=key_exp).get().get('Body').read()
